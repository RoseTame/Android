            ...
            new ActivityResultCallback<Map<String, Boolean>>() {
                ...
                public void onActivityResult(Map<String, Boolean> result) {
                    if (result.get(Manifest.permission.POST_NOTIFICATIONS) != null
                            ...
                            && result.get(Manifest.permission.FOREGROUND_SERVICE_MEDIA_PLAYBACK) != null) {
                        if (Objects.requireNonNull(result.get(Manifest.permission.POST_NOTIFICATIONS)).equals(true)
                                ...
                                && Objects.requireNonNull(result.get(Manifest.permission.FOREGROUND_SERVICE_MEDIA_PLAYBACK)).equals(true)) {
                            //权限全部获取到之后的动作
                            Log.d("flag", "获得所有相关权限");
                        } else {
                            //权限没有获取到的动作
